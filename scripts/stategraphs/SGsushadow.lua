    ...
    State{  name = "run",
			...
			events=
			{
				EventHandler("animover", function(inst) 
					...
					local hat = target ~= nil and target:GetDistanceSqToPoint(x, y, z) < 200 and target.components.inventory ~= nil and target.components.inventory:GetEquippedItem(EQUIPSLOTS.HEAD) and target.components.inventory:GetEquippedItem(EQUIPSLOTS.HEAD):HasTag("nightvision")
					
					
					if TheWorld.state.isnight and inst.components.combat:HasTarget() or (inst.running > 5 and (TheSim:GetLightAtPoint(x, y, z) == TUNING.DARK_CUTOFF or hat ~= nil and hat)) then
						inst.sg:GoToState("run")
					else
						inst.sg:GoToState("run_stop")
					end
