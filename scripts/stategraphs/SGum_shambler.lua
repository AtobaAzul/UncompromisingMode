...
local states =
{
    ...
    State{
        ...
        onenter = function(inst)
			...
			inst.AnimState:PushAnimation("aggro_norm")
    ...
    State{
        ...
        onenter = function(inst)
            if not (inst:CanStandUp() or inst.components.locomotor:WantsToMoveForward()) then
                inst.sg:GoToState("idle_hiding")
            elseif inst.sg.mem.is_hiding then
                inst.sg:GoToState("standup")
            else
                inst.Physics:Stop()
                inst.AnimState:PlayAnimation("aggroidle")
            end
        end,
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:PushAnimation("aggrowalk_loop")
        end,
