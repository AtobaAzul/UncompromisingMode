...
local function OnStartChannel(inst)
    ...
    if inst.components.timer:TimerExists("channel") then
        ...
        inst.components.timer:StartTimer("channel", TUNING.TOADSTOOL_MUSHROOMSPROUT_DURATION/3)
        OnTickChannel(inst)
    end
...
local states =
{
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(22 * FRAMES, function(inst)
                ...
                inst:DoMushroomBomb(inst.sg.statemem.targets) --This is actually the sheeplet summon code
    ...
    State{
        name = "mushroombomb", --Actually Twisted BAAa
        ...
        timeline =
        {
            ...
            TimeEvent(10 * FRAMES, function(inst)
				...
				inst.components.sanityaura.aura = -500*TUNING.SANITYAURA_HUGE
            end),
            TimeEvent(12 * FRAMES, function(inst)
                inst.sg:AddStateTag("nosleep")
                inst.sg:AddStateTag("nofreeze")
