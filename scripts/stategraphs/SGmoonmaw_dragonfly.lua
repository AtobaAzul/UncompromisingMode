...
local function onattackedfn(inst, data)
    ...
    elseif inst.sg:HasStateTag("crashed") and not inst.gotupalready then
        inst.sg:GoToState("getup")
        inst.gotupalready = true
    end
end
...
local function onattackfn(inst)
    if inst.components.health ~= nil and
        ...
        if inst.components.combat.target ~= nil and inst.components.combat.target:HasTag("structure") or
            math.random() < (inst.randattack * 0.05) or lavae ~= true then
            if lavae then
...
local states =
{
    ...
    State {
        ...
        onenter = function(inst)
            if inst.target ~= nil and ((inst.target ~= inst and not inst.target:HasTag("fire")) or inst.target == inst)
                and not (inst.recently_frozen or inst.flame_on) then
