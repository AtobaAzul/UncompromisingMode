--------------------------------------------------------------------------
--[[ Uncompromising_ShadowCreatureSpawner class definition ]] --------------------------------------------------------------------------
return Class(function(self, inst)

    assert(TheWorld.ismastersim, "Uncompromising shadow creature spawner should not exist on client")
    ...
    local function StopTracking(player, params, ent)
        ...
        if params.targetpop ~= #params.ents then
            if params.spawntask == nil then params.spawntask = player:DoTaskInTime(TUNING.SANITYMONSTERS_SPAWN_INTERVAL + TUNING.SANITYMONSTERS_SPAWN_VARIANCE * math.random(), UpdateSpawn, params) end
    ...
    local function StartTracking(player, params, ent)
        ...
        inst:ListenForEvent("onremove", function() if _players[player] == params then StopTracking(player, params, ent) end end, ent)

        inst:ListenForEvent("entitysleep", function() inst:DoTaskInTime(0, function() ent:Remove() end) end, ent)
    ...
    local function SpawnLandShadowCreature(player, spawndrifter)
        ...
        return SpawnPrefab(spawndrifter and "creepingfear" or player.components.sanity:GetPercent() < .5 and (random_chance < 0.2 and "terrorbeak" or random_chance < 0.8 and "dreadeye") or "crawlinghorror")
    end

    local function SpawnOceanShadowCreature(player, spawndrifter)
        local random_chance = math.random()

        return SpawnPrefab(spawndrifter and "creepingfear" or player.components.sanity:GetPercent() < .5 and random_chance < 0.2 and "dreadeye" or "oceanhorror")
    end
    ...
    function self:SpawnShadowCreature(player, params)
        ...
        -- THIS CODE IS WHAT MAKES THE CREEPING FEAR SPAWN
        ...
        for i, v in ipairs(params.ents) do -- a creepingfear already exist
            if v.prefab == "creepingfear" then spawndrifter = false end
        end
        ...
        for i, v in pairs(bosses) do if IsTargetingPlayer(v) then spawndrifter = false end end

        if player:HasTag("shadowdominant") or player:HasTag("fuelfarming") or bossesnearby > 0 or sanity > TUNING.DSTU.CREEPINGFEAR_SPAWN_THRESH then spawndrifter = false end
        ...
        if player.components.sanity:GetPercent() < .1 and boat ~= nil then
            ...
            if _map:IsOceanAtPoint(spawn_x, 0, spawn_z) then
                ...
                local ent = SpawnOceanShadowCreature(player, spawndrifter)
                ...
                if _failed_ocean_spawn_attempts >= OCEAN_SPAWN_ATTEMPTS then
                    if _map:IsPassableAtPoint(spawn_x, 0, spawn_z, false, false) then
                        ...
                        local ent = SpawnLandShadowCreature(player, spawndrifter)
                        ent.Transform:SetPosition(spawn_x, 0, spawn_z)
                        StartTracking(player, params, ent)
                    end
                end
    ...
    UpdateSpawn = function(player, params)
        if params.targetpop > #params.ents then
            ...
            params.spawntask = params.targetpop ~= #params.ents and player:DoTaskInTime(TUNING.SANITYMONSTERS_SPAWN_INTERVAL + TUNING.SANITYMONSTERS_SPAWN_VARIANCE * math.random(), UpdateSpawn, params) or nil
        elseif params.targetpop < #params.ents then
            -- Remove random monsters until we reach our target population
            local toremove = {}
            for i, v in ipairs(params.ents) do if not v.wantstodespawn and not (v.prefab == "creepingfear" and player.components.sanity:GetPercent() < 0.30) then table.insert(toremove, v) end end
    ...
    local function StartSpawn(player, params) if params.spawntask == nil then params.spawntask = player:DoTaskInTime(0, UpdateSpawn, params) end end
    ...
    local function UpdatePopulation(player, params)
        ...
        if is_insanity_mode and player.components.sanity.inducedinsanity then
            ...
            elseif math.random() < inc_chance then
                if targetpop < maxpop then targetpop = targetpop + 1 end
            ...
            params.poptask = player:DoTaskInTime(is_insanity_mode and (TUNING.SANITYMONSTERS_POP_CHANGE_INTERVAL + TUNING.SANITYMONSTERS_POP_CHANGE_VARIANCE * math.random()) or (NON_INSANITY_MODE_DESPAWN_INTERVAL + NON_INSANITY_MODE_DESPAWN_VARIANCE * math.random()), UpdatePopulation, params)
        end
    end

    local function Start(player, params) if params.poptask == nil then params.poptask = player:DoTaskInTime(0, UpdatePopulation, params) end end
    ...
    local function ForceSpawnShadow(inst)
        ...
        if math.random() < 0.5 then ent = SpawnPrefab("nightmarebeak") end
    ...
    local function OnPlayerJoined(inst, player)
        if _players[player] ~= nil then return end
    ...
    for i, v in ipairs(AllPlayers) do OnPlayerJoined(inst, v) end
    ...
    function self:GetDebugString()
        local count = 0
        for k, v in pairs(_players) do count = count + #v.ents end
        if count > 0 then return count == 1 and "1 shadowcreature" or (tostring(count) .. " shadowcreatures") end
    end
