...
--------------------------------------------------------------------------
--[[ Penguin(leech(scorpion))Spawner class definition ]]
--------------------------------------------------------------------------

return Class(function(self, inst)

assert(TheWorld.ismastersim, "Penguin(leech(scorpion))Spawner should not exist on client")
    ...
    local test = function(offset)
        ...
        return ground.Map:IsAboveGroundAtPoint(run_point:Get()) and
            ground.Pathfinder:IsClear(
                ...
                { ignorewalls = ignore_walls, ignorecreep = true }) and
		VALID_TILES[ground.Map:GetTileAtPoint(run_point:Get())] ~= nil
    end
...
local function SpawnScorpion(inst,spawner,colonyNum,pos,angle)

    local scorpionprefab = "scorpion"

    local scorpion = SpawnPrefab(scorpionprefab)
    if scorpion then
        --print(TheCamera:GetHeading()," spawnPenguin at",pos,"angle:",angle)

        scorpion.Transform:SetPosition(pos.x,pos.y,pos.z)
        scorpion.Transform:SetRotation(angle)
...
local function SpawnSwarm(colonyNum,loc,check_angle)
    ...
    while spawned < flock and i < flock + 7 do
        ...
        if map:IsAboveGroundAtPoint(spawnPos:Get()) then
            ...
            --print(TheCamera:GetHeading()," spawnPenguin at",pos,"angle:",angle)
            self.inst:DoTaskInTime(GetRandomWithVariance(1,1), SpawnScorpion, self, colonyNum, spawnPos,(check_angle/DEGREES))
        end
    end
end
