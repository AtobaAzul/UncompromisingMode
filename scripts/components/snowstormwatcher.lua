...
function TrySpawning(v)
	...
	if TheWorld.state.iswinter and
		...
		TheWorld.Map:GetPlatformAtPoint(x1, 0, z1) == nil then --and self.snowstormstart then
...
local function SnowpileChance(inst, self)
	...
	if TheWorld.state.iswinter and
		((TheWorld.net ~= nil and TheWorld.net:HasTag("snowstormstartnet")) or TheWorld:HasTag("snowstormstart")) then --and self.snowstormstart then
