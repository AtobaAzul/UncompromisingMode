...
assert(TheWorld.ismastersim, "CaveDeerclopsspawner should not exist on client")
...
local function ReleaseHassler(targetPlayer)
    ...
    local spawn_pt = targetPlayer:GetPosition()
    if spawn_pt ~= nil then
        ...
        if hassler ~= nil then
            hassler.Physics:Teleport(spawn_pt.x, 35, spawn_pt.z)
			hassler.sg:GoToState("fall")
