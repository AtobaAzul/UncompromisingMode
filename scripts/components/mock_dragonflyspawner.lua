...
return Class(function(self, inst)
        assert(TheWorld.ismastersim, "Mock_dragonflyspawner should not exist on client")
        ...
        function self:GetDebugString()
            ...
            elseif _timetoattack > 0 then
                s = s ..
                    string.format("%s Dragonfly is coming for %s in %2.2f", _warning and "WARNING" or "WAITING",
                        tostring(_targetplayer) or "<nil>", _timetoattack)
            else
