JukeAndJive = Class(BehaviourNode, function(self, inst, hunterparams, see_dist, safe_dist, fn, runhome, fix_overhang, walk_instead, safe_point_fn)
    BehaviourNode._ctor(self, "JukeAndJive")
...
function JukeAndJive:__tostring()
    return string.format("JukeAndJive %f from: %s", self.safe_dist, tostring(self.hunter))
end



function JukeAndJive:GetRunAngle(pt, hp, sp)
	...
	if sp ~= nil then
		...
		angle = (self.inst:GetAngleToPoint(hp) + 180) + self.moddedangle
		if angle > 360 then
			angle = angle - 360
		end
	end
...
function JukeAndJive:PickNewDirection()
	self.moddedangle = math.random() > 0.5 and -50 or 50
    self.waittime = GetTime() + 5 + math.random()
end


function JukeAndJive:Visit()
