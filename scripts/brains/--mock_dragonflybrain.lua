...
local function SleepAction(inst)
    if ((inst.num_ashes_eaten >= 20) or (inst.num_targets_vomited >= 40)) 
...
local Mock_dragonflybrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
end)

function Mock_dragonflybrain:OnStart()
    local root =
        PriorityNode(
        {
