...
local Spider_CrabbitBrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
    self.mytarget = nil
end)

function Spider_CrabbitBrain:SetTarget(target)
    if target ~= nil then
...
function Spider_CrabbitBrain:OnStop()
    self:SetTarget(nil)
end
...
function Spider_CrabbitBrain:OnStart()
    ...
    local root = PriorityNode(
    {
        WhileNode(function() return not self.inst.sg:HasStateTag("teleporting") end, "TeleportBlock",
			...
			Wander(self.inst, function() return self.inst.components.knownlocations:GetLocation("home") end, 12)
            }, .25)),
    }, .25)

    self.bt = BT(self.inst, root)
end
function Spider_CrabbitBrain:OnInitializationComplete()
    self.inst.components.knownlocations:RememberLocation("home", Point(self.inst.Transform:GetWorldPosition()))

end
return Spider_CrabbitBrain
