...
local function OnBrushed(inst, doer, numprizes)
    if numprizes > 0 and inst.components.domesticatable ~= nil then
        inst.components.domesticatable:DeltaDomestication(TUNING.BEEFALO_DOMESTICATION_BRUSHED_DOMESTICATION*100) --Make fully domesticating easy for testing
        inst.components.domesticatable:DeltaObedience(TUNING.BEEFALO_DOMESTICATION_BRUSHED_OBEDIENCE*100)
    end
end
...
local function SetTendency(inst, changedomestication)
    ...
    --[[if not inst.components.domesticatable:IsDomesticated() then -- Moon beefalo don't have tendencies
        local tendencysum = 0
...
return Prefab("beefalo_moon", beefalo)
