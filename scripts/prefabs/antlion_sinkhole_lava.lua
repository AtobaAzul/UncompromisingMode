...
local function donextcollapse(inst)
    ...
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, 0, z, TUNING.ANTLION_SINKHOLE.RADIUS + 1 / 1.5, nil, inst.collapsestage > 1 and NON_COLLAPSIBLE_TAGS or NON_COLLAPSIBLE_TAGS_FIRST, COLLAPSIBLE_TAGS)
    ...
    local totoss = TheSim:FindEntities(x, 0, z, TUNING.ANTLION_SINKHOLE.RADIUS / 1.5, { "_inventoryitem" }, { "locomotor", "INLIMBO" })
...
local function onstartcollapse(inst)
    ...
    inst.collapsetask = inst:DoPeriodicTask(COLLAPSE_STAGE_DURATION / 1.5, donextcollapse)
    donextcollapse(inst)
end
...
local function fn()
    ...
    inst.components.unevenground.radius = TUNING.ANTLION_SINKHOLE.UNEVENGROUND_RADIUS / 1.5
...
return Prefab("antlion_sinkhole_lava", fn, assets, prefabs)
