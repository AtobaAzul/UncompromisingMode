...
local function TryEjectLavae(inst, severed)
	if inst.components.health ~= nil and not inst.components.health:IsDead() and NoLavae(inst) == false and
		inst.components.leader:CountFollowers() == 0 then
...
local function PerEjectCheck(inst)
	if inst.components.combat ~= nil and inst.components.combat.target ~= nil and
		(inst.components.health ~= nil and not inst.components.health:IsDead()) then
...
local function fn(Sim)
	...
	inst.components.health:SetMaxHealth(TUNING.DSTU.WILTFLY_HEALTH * 2)
