...
local function FindClosestPart(inst)
    ...
    if ents ~= nil then
        ...
        inst.tracking_parts = {}
        for k, v in pairs(ents) do if v:HasTag("player") then table.insert(inst.tracking_parts, v) end end
        -- end
    end
...
local function OnWake(inst) if inst.task == nil then inst.task = inst:DoTaskInTime(1, CheckTargetPiece) end end
...
local function getstatus(inst) if inst.sg:HasStateTag("invisible") then return "HIDDEN" end end
...
local function DoSpawnSpikes(inst, pts, level)
    if not inst.components.health:IsDead() then
        for i, v in ipairs(pts) do
            ...
            if #TheSim:FindEntities(x, y, z, 5, {"structure", "tree", "boulder"}) > 0 then spike:Remove() end
        end
    end
end
