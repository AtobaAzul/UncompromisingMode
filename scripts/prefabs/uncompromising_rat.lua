...
local function OnAttackOther(inst, data)
	if data.target ~= nil and data.target:HasTag("player") and not data.target:HasTag("hasplaguemask") and
		not data.target:HasTag("ratfriend") and not data.target:HasTag("automaton") and TUNING.DSTU.MAXHPHITTERS then
...
local function KeepTargetFn(inst, target)
	local validitem = target.components.inventory ~= nil and
		target.components.inventory:FindItem(function(item) return not item:HasTag("nosteal") end)
...
local function onopen(inst, data)
	if TheWorld.components.winkyburrowinventory.trunk and
		not TheWorld.components.winkyburrowinventory.trunk.components.container:IsOpen() then
...
local function IsAVersionOfRot(v)
	if v.prefab == "spoiled_food" or v.prefab == "rottenegg" or v.prefab == "spoiled_fish" or
		v.prefab == "spoiled_fish_small" then
		return true
	end
end
