...
local function rattargetfn(inst)
    return FindEntity(inst, 3, function(guy)
        ...
        return inst:GetTimeAlive() > 5 and not inst:HasTag("carrying") and guy:HasTag("player") and validitem ~= nil and inst.components.combat:CanTarget(guy) and not (inst.components.follower ~= nil and inst.components.follower.leader == guy)
    end, nil, RETARGET_CANT_TAGS) or nil
end
...
local function junkretargetfn(inst)
    ...
    if inst.shouldhide == false then
        ...
        if #rats > 5 then -- Only try and target the player this way if there's a bunch of rats nearby
            ...
            if victim ~= nil then
                for i, v in ipairs(rats) do if v.components.combat ~= nil and v.components.combat.target == nil then v.components.combat:SuggestTarget(victim) end end
                return victim
            else
                return nil
            end
    ...
    elseif inst.sg:HasStateTag("hiding") then
        local victim = FindEntity(inst, 12, function(guy) return inst.components.combat:CanTarget(guy) end, nil, RETARGET_CANT_TAGS)
        if victim ~= nil then
...
local function OnJunkAttacked(inst, data)
    ...
    if inst.task ~= nil then inst.task:Cancel() end
...
local function FindTargetOfInterest(inst)
    ...
    if inst.shouldhide == false then
        ...
        if math.random() > 0.25 then inst.shouldhide = true end
    else
...
local function EndRaid(inst)
    ...
    for rats, _ in pairs(inst.components.herd.members) do end
...
local function onsave_winkyburrow(inst, data)
    if inst.ratcount ~= nil then data.ratcount = inst.ratcount end

    if inst.myowner ~= nil then data.myowner = inst.myowner end
end

local function onload_winkyburrow(inst, data)
    if data ~= nil then
        if data.ratcount ~= nil then inst.ratcount = data.ratcount end

        if data.myowner ~= nil then inst.myowner = data.myowner end
    end
end
...
local function onopen(inst, data) inst.AnimState:PlayAnimation("dig") end

local function onclose(inst, data) inst.AnimState:PlayAnimation("dig") end

local function GetActivateVerb(inst) return "OPEN" end

local function AttachShadowContainer(inst) inst.components.container_proxy:SetMaster(TheWorld:GetPocketDimensionContainer("winky")) end
...
local function CheckIfEmpty(inst) if inst.components.herd.membercount == 0 then inst:Remove() end end
...
local function fn_droppings()
    ...
    inst:AddComponent("inventoryitem")
    inst.components.inventoryitem.nobounce = true
    inst.components.inventoryitem:SetOnPutInInventoryFn(function(inst) inst:DoTaskInTime(.5, inst.Remove) end)
...
local function fn_warning()
    ...
    inst:DoTaskInTime(0, function() PlayWarningSound(inst) end)
