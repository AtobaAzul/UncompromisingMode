...
local function oncollide(inst, other)
    local x, y, z = inst.Transform:GetWorldPosition()
    if other ~= nil and not other:HasTag("walrus") and not other:HasTag("hound") and other:IsValid() and
        other:HasTag("_combat") then
...
local function Spawntrap(inst)
    ...
    if TheWorld.state.iswinter then
        ...
        if #TheSim:FindEntities(x, y, z, 10, { "bear_trap" }) < 7 and
            #TheSim:FindEntities(x, y, z, 30, { "structure" }, { "webbedcreature" }) < 20 and
            #TheSim:FindEntities(x, y, z, 40, { "player" }) == 0 then
            DoSpawnTrap(x, y, z)
        end
