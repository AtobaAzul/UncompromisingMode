...
local function Explode(inst)
	...
	SpawnPrefab("toadling").Transform:SetPosition(x, y, z)
    local ents = TheSim:FindEntities(x, y, z, TUNING.TOADSTOOL_MUSHROOMBOMB_RADIUS, { "_health", "_combat" }, { "INLIMBO", "toadling" })
    if #ents > 0 then
...
local function MakeBomb(name)
    ...
    local function fn()
		...
		inst.Transform:SetScale(2.8, 2, 2)
...
return MakeBomb("toadlingspawner")
