...
local function doprojectilehit(inst, other)
	...
	local other = other or FindEntity(inst, TUNING.WARG_GOO_RADIUS * 1.25, { "_combat", "player" }, { "INLIMBO", "shadow", "shadowdominance" })
	if other ~= nil and other ~= caster and other.components.combat ~= nil  then
        if other.components.sanity ~= nil and (other.components.health == nil or not other.components.health:IsDead()) and other.components.sanity:IsInsane() and other.components.inkable and not other.components.inventory:EquipHasTag("shadowdominance") then
		...
		elseif other.components.inkable and not other.components.inventory:EquipHasTag("shadowdominance") then
            other.components.inkable:Ink()
			DoSplatFx(inst)
        end
	else
