    ...
    for i = 1,20 do
        inst:DoTaskInTime(FRAMES * i --[[* 1.5]], function()
            ...
            inst:DoTaskInTime(.6, function()
                ...
                local ents = TheSim:FindEntities(dx, dy, dz, 1.5, nil, { "FX", "NOCLICK", "INLIMBO", "shadowdominance" })
                for k,v in ipairs(ents) do
                    if not target_index[v] and v ~= inst and inst.components.combat:IsValidTarget(v) and v.components.combat and ((v.components.sanity and v.components.sanity:IsInsane()) or v == target) and not v:HasTag("shadowdominance") then
                        target_index[v] = true
...
local function CreepinFearTimer(inst)
    ...
    if inst.sp_atk_cd <= 0 and inst.components.combat:HasTarget() and not inst.sg:HasStateTag("busy") then
        if target:IsValid() and not target:HasTag("shadowdominance") then
