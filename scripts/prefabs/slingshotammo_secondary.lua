...
local function AoE_Speed(inst)
    ...
    for i, v in pairs(ents) do
        if v ~= nil and v ~= inst and v:IsValid() and v.components.locomotor ~= nil then
			...
			v._slingshot_speedmulttask = v:DoTaskInTime(TUNING.SLINGSHOT_AMMO_MOVESPEED_DURATION / 3, function(i) i.components.locomotor:RemoveExternalSpeedMultiplier(i, debuffkey) i._slingshot_speedmulttask = nil end)
...
local function OnHit_Marble(inst, attacker, target)
	...
	if target ~= nil and target:IsValid() and target.components and 
		target.components.locomotor and not target:HasTag("stageusher") and not 
		target:HasTag("toadstool") then
