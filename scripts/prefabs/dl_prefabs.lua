...
local function GetKeyFromValue(t, value)
    ...
    for k, v in ipairs(ents) do
        if v ~= inst then
            ...
            if v.prefab == "dl_tileflag" then
                thedata.tile = GetKeyFromValue(WORLD_TILES, TheWorld.Map:GetTileAtPoint(vx, vy, vz))
                ...
                if v.tile ~= nil then
                    ...
                    if not spawn_in_water and TheWorld.Map:IsPassableAtPoint(px, v.relative_y + y, pz) or spawn_in_water then
                        if v.tile == WORLD_TILES.MONKEY_DOCK then
                            ...
                            TheWorld.Map:SetTile(tile_x, tile_z, WORLD_TILES[v.tile], { group = group, reversible = reversible })
                        end
                    end
                else
