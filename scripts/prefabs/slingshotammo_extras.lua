...
local function no_aggro(attacker, target)
    ...
    return targets_target ~= nil and targets_target:IsValid() and targets_target ~= attacker and (GetTime() - target.components.combat.lastwasattackedbytargettime) < 4 and (targets_target.components.health ~= nil and not targets_target.components.health:IsDead()) -- this can return nil, right?
end
...
local function Beat(inst) inst.SoundEmitter:SetParameter("shadowvortex", "intensity", inst.Transform:GetScale() / 1.8) end
...
local function Disappear_mini(inst) inst.components.sizetweener:StartTween(0.05, 0.1, inst.Remove) end
...
local function shrinktask_mini(inst) inst:DoTaskInTime(inst.powerlevel * 2, shrink_mini) end
...
local function GlassCut(inst)
    ...
    for i, v in ipairs(TheSim:FindEntities(x, y, z, 2, "_combat", cant_tags)) do
        if v:GetPhysicsRadius(0) <= 1.5 and v:IsValid() and v.components.combat ~= nil and v.components.combat ~= nil and v.components.health ~= nil and not (v.sg ~= nil and (v.sg:HasStateTag("swimming") or v.sg:HasStateTag("invisible"))) and (v:HasTag("bird_mutant") or not v:HasTag("bird")) then
            if not (v.components.follower ~= nil and v.components.follower:GetLeader() ~= nil and v.components.follower:GetLeader():HasTag("player")) then
                if not (v.components.health:IsDead() or v == attacker or v:HasTag("playerghost") or (v:HasTag("player") and not TheNet:GetPVPEnabled())) then
                    inst.finallevel = inst.powerlevel
...
local function Advance(inst)
    ...
    if inst.target ~= nil and inst.target:IsValid() and inst.target.components.locomotor ~= nil then
        ...
        inst.target._slingslime_speedmulttask = inst.target:DoTaskInTime(30, function(i)
            i.components.locomotor:RemoveExternalSpeedMultiplier(i, debuffkey)
            i._slingslime_speedmulttask = nil
            inst:Remove()
            i.slingshot_slime = nil
        end)
...
local function shadowclone_fn()
    ...
    inst:DoTaskInTime(30, function(inst) inst.components.health:Kill() end)

    inst:ListenForEvent("attacked", function(inst) inst.SoundEmitter:PlaySound("dontstarve/impacts/impact_sanity_armour_dull") end)
