...
local AURA_EXCLUDE_TAGS = { "playerghost", "ghost", "shadow", "shadowminion", "noauradamage", "INLIMBO", "notarget", "noattack", "invisible" }
...
local function SpawnOverlayFX(inst, i, set, isnew)
    ...
    local fx = SpawnPrefab("wixie_stinkcloud_overlay")
...
local function CreateBase(isnew)
	...
	inst.AnimState:SetMultColour(0.6, 0.4, 0.3, 0.7)
...
local function KillOverlayFX(fx)
	fx.SoundEmitter:KillSound("flies")
...
local function TryPerish(target)
	...
	target:AddDebuff("wixie_stinkdebuff", "wixie_stinkdebuff")
end

local function DoAreaSpoil(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, inst.components.aura.radius, nil, AURA_EXCLUDE_TAGS, {"_combat"})
    for i, v in ipairs(ents) do
		if v:HasTag("player") then
			if v.components.sanity ~= nil then
				v.components.sanity:DoDelta(-0.1)
			end
		else
			TryPerish(v)
		end
...
return Prefab("wixie_stinkcloud", fn, assets, prefabs),
    Prefab("wixie_stinkcloud_overlay", overlayfn, assets)