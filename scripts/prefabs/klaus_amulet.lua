...
local function DoubleSlap(owner)
	...
	if not owner.components.rider:IsRiding() and equip ~= nil and equip.components.weapon ~= nil and
		...
		local damagemultex = owner.components.combat.externaldamagemultipliers ~= nil and
			owner.components.combat.externaldamagemultipliers:Get() or 1
...
local function onequip_blue(inst, owner)
	if not owner:HasTag("vetcurse") then
		inst:DoTaskInTime(0, function(inst, owner)
			...
			local tool = owner ~= nil and
				(
				owner.components.inventory:GetEquippedItem(EQUIPSLOTS.BODY) or
					owner.components.inventory:GetEquippedItem(EQUIPSLOTS.NECK))
