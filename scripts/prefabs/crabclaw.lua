...
local function onattack(inst, owner, target)
	...
	if redgem > 0 and owner.components.health ~= nil and owner.components.health:GetPercent() < 1 and
	...
	if yellowgem > 0 and owner.components.sanity ~= nil and owner.components.sanity:GetPercent() < 1 and
		not (target:HasTag("wall") or target:HasTag("engineering")) then
	...
	if bluegem > 0 and target:IsValid() and target.components.combat ~= nil and target.components.freezable ~= nil and
		not target.components.health:IsDead() and not target.components.freezable:IsFrozen() then
