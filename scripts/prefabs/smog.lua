...
local SMOG_SPACING_RADIUS = SQRT2 * (TUNING.MIASMA_SPACING*0.75) * TILE_SCALE
local SMOG_PARTICLE_RADIUS = SMOG_SPACING_RADIUS*0.66
...
local _SmogCloudEntities = nil -- Tracking the entities themselves for TheCamera; client side.
...
local function OnCameraUpdate_Client(dt)
    ...
    if heading ~= _OldHeading then
        ...
        for smogcloud, _ in pairs(_SmogCloudEntities) do
            smogcloud._front_cloud_fx.Transform:SetPosition(ox, 0.1, oz)
            smogcloud._back_cloud_fx.Transform:SetPosition(-ox, 0.1, -oz)
        end
    end
end
...
local function SetupParticles(inst)
    ...
    local particles_per_tick = 6 * TheSim:GetTickTime() -- Half intensity with particle placement folding.
    local num_to_emit = 0
...
local function fn()
    ...
    inst:DoPeriodicTask(5 + math.random(5), function(inst)
        ...
        for k, v in ipairs(ents) do
            if v.components.health ~= nil and  math.random() > 0.5 then
