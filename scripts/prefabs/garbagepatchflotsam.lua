...
local function OnLand(inst)
    ...
    if land_in_water then
        ...
        local item = SpawnPrefab("garbagepatchflotsam")
        item.Transform:SetPosition(x, y, z)
        item.Transform:SetRotation(inst.Transform:GetRotation())
        inst.components.inventory:TransferInventory(item)
...
local function OnSalvage(inst)
    local product = SpawnPrefab("garbagepatchflotsam")
...
local function waterfn(data)
    ...
    inst:AddTag("garbagepatchflotsam")
...
local function OnSink(inst)
    local water = SpawnPrefab("garbagepatchflotsam_water")

    water.Transform:SetPosition(inst.Transform:GetWorldPosition())
    inst.components.inventory:TransferInventory(water)
...
return Prefab("garbagepatchflotsam_water", waterfn, assets, water_prefabs),
    Prefab("garbagepatchflotsam", landfn, assets, land_prefabs)
