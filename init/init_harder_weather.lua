...
function trip_over(player)
    ...
    player:PushEvent("knockback", {knocker = player, radius = GLOBAL.TUNING.DSTU.TRIPOVER_KNOCKABCK_RADIUS})
    player.components.health:DeltaPenalty(GLOBAL.TUNING.DSTU.TRIPOVER_HEALTH_PENALTY)
end
...
local function trip_over_chance_on_maxwet(player)
    ...
    if(player~=nil and player.GetMoisture == self.GetMaxMoisture) then
        print("totally wet")
        if (GLOBAL.math.random() > GLOBAL.TUNING.DSTU.TRIPOVER_ONMAXWET_CHANCE)-- and
            --~player.walking) then
