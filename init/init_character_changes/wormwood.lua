...
local function ForceBloomTimeout(inst)
	if inst.forcedbloom then
		inst.forcedbloom = false
		if not TheWorld.state.isspring then
			inst.components.talker:Say(GetString(inst, "ANNOUNCE_HAYFEVER_OFF"))
			SpawnBloomFX(inst)
...
local function ForceBloom(inst)
    if not inst.forcedbloom then
        inst.components.talker:Say(GetString(inst, "ANNOUNCE_BLOOMING"))
		inst.forcedbloom = true
...
env.AddPrefabPostInit("wormwood", function(inst)
	...
	inst:ListenForEvent("forcebloom", function(inst) ForceBloom(inst) end)
