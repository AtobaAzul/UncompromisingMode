----------------------------------------------------------------------------------
-- Insanity overlay effect while at 0 sanity (to reduce screen visibility)
-- Copied from mindcontrolover.lua
-- Used in ini_harder_shadows
----------------------------------------------------------------------------------
...
local ZeroInsanityOverlay = Class(UIAnim, function(self, owner)
    self.owner = owner
    UIAnim._ctor(self)
    ...
    if owner ~= nil then
        -- Use it as such: parent:PushEvent("insanitylevel", inst._level:value() / MAX_LEVEL)
        self.inst:ListenForEvent("insanitylevel", function(owner, level) self:PushLevel(level) end, owner)
    end
end)
...
function ZeroInsanityOverlay:PushLevel(level)
...
function ZeroInsanityOverlay:OnUpdate(dt)
    if self.targetlevel > 0 then
        --wot! bad state
        self:StopUpdating()
