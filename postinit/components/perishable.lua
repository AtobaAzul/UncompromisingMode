...
env.AddComponentPostInit("perishable", function(self)
	...
	function self:SetPerishTime(time)
		if self.inst.components.perishable and (self.inst.components.edible or self.inst:AddComponent("cookable")) and not self.inst.components.equippable and not self.inst:HasTag("lightbattery") then
