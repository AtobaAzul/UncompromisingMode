...
env.AddComponentPostInit("playerspawner", function(self)
    function self:_ShouldEnableSpawnProtection(inst, player, x, y, z, isloading)
        if TheWorld.topology.overrides ~= nil and TheWorld.topology.overrides.spawnprotection == "never" then return false end
        return true
    end
    ...
    function self:SpawnAtLocation(inst, player, x, y, z, isloading, platform_uid, rx, ry, rz)
        if player:HasTag("troublemaker") and not isloading and TheSim:FindFirstEntityWithTag("wixie_wardrobe") ~= nil then
            ...
            player:DoTaskInTime(6 * FRAMES + FRAMES, function(inst)
                ...
                if not TheWorld.state.isday and #TheSim:FindEntities(ux, uy, uz, 4, SPAWNLIGHT_TAGS) <= 0 then SpawnPrefab("spawnlight_multiplayer").Transform:SetPosition(ux, uy, uz) end
            end)
