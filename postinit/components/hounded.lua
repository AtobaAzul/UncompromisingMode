...
if not GLOBAL:TestForIA() then
    AddComponentPostInit("hounded", function(self)
        ...
        if TUNING.DSTU.SPOREHOUNDS and not GLOBAL:TestForIA() then
            self.seasonal_prefabs["autumn"] = { "sporehound" }
        end

        if TUNING.DSTU.GLACIALHOUNDS and not GLOBAL:TestForIA() then
            self.seasonal_prefabs["winter"] = { "glacialhound" }
        end

        if TUNING.DSTU.LIGHTNINGHOUNDS and not GLOBAL:TestForIA() then
            self.seasonal_prefabs["spring"] = { "lightninghound" }
        end

        if TUNING.DSTU.MAGMAHOUNDS and not GLOBAL:TestForIA() then
