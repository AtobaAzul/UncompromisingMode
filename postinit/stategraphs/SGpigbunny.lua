...
{	
	...
	EventHandler("attacked", function(inst)
		...
		if inst:HasTag("pigattacker") and inst:HasTag("pig") and not inst:HasTag("werepig") and inst.components.health ~= nil and not inst.components.health:IsDead() then
	...
	State{
        ...
        timeline =
        {
            TimeEvent(9 * FRAMES, function(inst)
				...
				if target ~= nil and inst.components.combat:CanHitTarget(target) then--distsq(target:GetPosition(), inst:GetPosition()) <= 10 then
