...
local function FindSpotForShadow(inst, target, shadow)
	if not (target and target:IsValid()) then
		...
		if queen and queen.prioritytarget and queen.prioritytarget.components.health and not queen.prioritytarget.components.health:IsDead() then target = queen.prioritytarget end
	end
	local distance = 7
...
local function UpdateShadowShooter(inst)
	if inst.bee then
		local x, y, z = inst.bee.Transform:GetWorldPosition()
		if y ~= nil and y > 0.5 then
			if inst.circle then inst.Transform:SetPosition(x, 0, z) end
			local scaleFactor = Lerp(.5, 1.5, y / 35)
			inst.Transform:SetScale(scaleFactor, scaleFactor, scaleFactor)
...
local function Pop(inst)
	...
	if inst.queen ~= nil and inst.queen.prefab == "cherry_beequeen" then honey.AnimState:SetHue(0.7) end
...
env.AddStategraphPostInit("SGbeeguard", function(inst) -- beeguard time
	...
	local events = {}
	local states = {State {name = "flyup", tags = {"busy", "nosleep", "nofreeze", "noattack", "flight", "mortar"}, onenter = function(inst)
		...
		if y > inst.maxflyheight then inst.sg:GoToState("flydown") end
	end}, State {name = "flyup_shooter", tags = {"busy", "nosleep", "nofreeze", "noattack", "flight", "mortar"}, onenter = function(inst)
		SpawnPrefab("bee_poof_small").Transform:SetPosition(inst.Transform:GetWorldPosition())
		if inst.SoundEmitter:PlayingSound("buzz") then
			inst.SoundEmitter:KillSound("buzz")
			inst.SoundEmitter:PlaySound(inst.sounds.buzz, "buzz")
		...
		inst.Physics:SetMotorVel(inst.sg.statemem.vel:Get())
		if y > inst.maxflyheight then inst.sg:GoToState("flydown_shooter") end
	end}, State {name = "flydown", tags = {"busy", "nosleep", "nofreeze", "noattack", "flight", "mortar"}, onenter = function(inst)
		StopCollide(inst)
		SpawnPrefab("bee_poof_small").Transform:SetPosition(inst.Transform:GetWorldPosition())
		if inst.SoundEmitter:PlayingSound("buzz") then
			inst.SoundEmitter:KillSound("buzz")
		...
		local queen = inst.components.entitytracker:GetEntity("queen")
		if queen and queen.prioritytarget and queen.prioritytarget.components.health and not queen.prioritytarget.components.health:IsDead() then inst.stabtarget = queen.prioritytarget end
	...
	end, onexit = function(inst)
		StartCollide(inst)
	end}, State {name = "flydown_shooter", tags = {"busy", "nosleep", "nofreeze", "noattack", "flight", "mortar"}, onenter = function(inst)
		StopCollide(inst)
		if inst.circle then inst.CircleFormation(inst) end
		SpawnPrefab("bee_poof_small").Transform:SetPosition(inst.Transform:GetWorldPosition())
		if inst.SoundEmitter:PlayingSound("buzz") then
			inst.SoundEmitter:KillSound("buzz")
			inst.SoundEmitter:PlaySound(inst.sounds.buzz, "buzz")
		...
		if inst.circle then StartCollide(inst) end
	end}, State {name = "stab", tags = {"busy", "nosleep", "nofreeze", "attack", "noattack", "mortar"}, -- We don't want the beeguard to try and attack, but we do need to let the game know this is an attacking state.
	onenter = function(inst)
		...
		inst.stuck = nil
	end}, State {name = "stuck", tags = {"busy", "stuck"}, onenter = function(inst)
		StartBuzz(inst)
	...
	end, timeline = {TimeEvent(10 * FRAMES, function(inst)
		StopBuzz(inst)
	end)}, events = {EventHandler("animqueueover", function(inst)
	...
	end)}}, State { -- Need the shooter bees to stand still when they're going to shoot in a line
	name = "lineshoot", tags = {"busy", "stuck"}, onenter = function(inst)
		StartBuzz(inst)
		inst.SoundEmitter:PlaySound(inst.sounds.hit)
		inst.components.locomotor:StopMoving()
		inst.AnimState:PlayAnimation("idle", true)
	end}, State {name = "shoot_pre", tags = {"busy"}, onenter = function(inst)
		if inst.target and inst.target:IsValid() then inst:ForceFacePoint(inst.target:GetPosition()) end
		inst.SoundEmitter:PlaySound(inst.sounds.attack)
		if inst.circle then
		...
		if inst.components.linearcircler and inst.components.linearcircler.distance < 12 then inst.components.linearcircler.distance = 0.1 + inst.components.linearcircler.distance end
	end}, State {name = "charge", -- CHARGE! Beeguards charge at the player in formation.
	...
	onenter = function(inst)
		...
		local stabbed = FindEntity(inst, 1, function(guy)
			for i, ent in ipairs(inst.alreadyStabbed) do if guy == ent then return false end end
			return true
		end, {"_combat"}, {"bee", "shadow", "beehive"})
		if stabbed then
	...
	end}, State {name = "hold_position", -- All this does is make the beeguard stick on a single position after he finishes charging
	...
	onenter = function(inst)
		...
		if inst.holdPoint then inst.Transform:SetPosition(inst.holdPoint.x, inst.holdPoint.y, inst.holdPoint.z) end
	end, onexit = function(inst)
		StartCollide(inst)
		inst.brain:Start()
		inst.holding = false
	end}, State {name = "hold_position_ring", -- All this does is make the beeguard stick on a single position after he finishes charging
	tags = {"busy"}, onenter = function(inst)
		inst.brain:Stop()
		StopCollide(inst)
		inst.AnimState:PlayAnimation("idle", true)
		inst.holding = true
		...
		StartCollide(inst)
		inst.holding = false
	end}, State {name = "rally_at_point", -- Similar to CHARGE but doesn't do damage, bees are just getting ready to charge
	tags = {"attack", "busy", "nofreeze", "nosleep", "noattack", "flight", "moving"}, -- Tags galore...
	onenter = function(inst)
		StopCollide(inst)
		inst.brain:Stop()
		...
		if inst.beeHolder and inst.beeHolder:IsValid() then
			...
			if inst:IsValid() and inst:GetDistanceSqToPoint(position) < 1 then inst.sg:GoToState("hold_position_ring") end
		else
	...
	end}, State {name = "defensiveattack", tags = {"attack", "busy", "caninterrupt"}, onenter = function(inst)
		inst.components.locomotor:StopMoving()
	...
	end), TimeEvent(21 * FRAMES, function(inst)
		inst.sg:RemoveStateTag("busy")
	end)}, onupdate = function(inst)
		if inst.components.combat and inst.components.combat.target then inst:ForceFacePoint(inst.components.combat.target:GetPosition()) end
		if inst.beeHolder and inst.beeHolder:IsValid() then
			local x, y, z = inst.beeHolder.Transform:GetWorldPosition()
			inst.Transform:SetPosition(x, y, z)
		end
